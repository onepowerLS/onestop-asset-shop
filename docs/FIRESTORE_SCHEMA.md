...
| `capabilities` | map | **Retired 2026-08-11** — no longer consulted for authorization; the signed Nexus action ladder (`operate_assets` / `approve_assets`) governs. |
| `amCountryAccess` | array of strings | **Legacy fallback only** (non-SSO sessions). Authorization scope now comes from the signed Nexus claim's `scopeCountries` (ISO-2 `LS`/`ZM`/`BJ`, normalized internally to `LSO`/`ZMB`/`BEN`). An empty claim scope is a **global** grant (all org countries), not "no access". |

**Capabilities:** retired. SIM team assignment and SIM↔phone linking require `operate_assets` (Level C); IT and AM ops queue management require `approve_assets` (Level B). Old boolean flags on `users/{uid}` are inert data.
